...
def u_A(a, omega_0, omega, delta, C, u_a, u_omega_0, u_omega, u_delta, u_B, u_C):
    # this function is the result of a translation from a Latex equation to python expression
    denom1 = 4 * delta**2 * omega**2 + (omega_0**2 - omega**2)**2

    term_a = (1 / np.sqrt(denom1) * u_a)**2

    term_omega_0 = (
        (-2 * a * omega_0 * (omega_0**2 - omega**2))
        / (denom1**(3/2))
        * u_omega_0
    )**2

    term_omega = (
        (-2 * a * omega * (-omega_0**2 + 2 * delta**2 + omega**2))
        / ((omega_0**4 - 2 * omega_0**2 * omega**2 + 4 * delta**2 * omega**2 + omega**4)**(3/2))
        * u_omega
    )**2
    ...
    return np.sqrt(term_a + term_omega_0 + term_omega + term_delta + u_B**2)

def fit_func(params:list, omega):
...
def evaluation(frequency:list, amplitude:list, label, fig, axs):
    fit_func_instance = odr.Model(fit_func)
    data = odr.RealData(frequency, amplitude, sx = uncertainty_frequency, sy = uncertainty_amplitude)
    odr_fit = odr.ODR(data,fit_func_instance, beta0 = [400, 3.3, 0.2, 0, 0])
    output = odr_fit.run()

    output.pprint()

    a, omega_0, delta, B, C = output.beta[0], output.beta[1], output.beta[2], output.beta[3], output.beta[4]
    u_a, u_omega_0, u_delta, u_B = output.sd_beta[0], output.sd_beta[1], output.sd_beta[2], output.sd_beta[3]

    x = np.linspace(min(frequency), max(frequency), 10000)
    y = [fit_func([a, omega_0, delta, B, C], i) for i in x]
    ...
    amplitude_res_freq = fit_func([a, omega_0, delta, B, C], resonance_frequency)
    u_amplitude_res_freq = u_A(a, omega_0, resonance_frequency, delta, u_a, u_omega_0, u_omega_0, u_delta, u_B)
    ...
    func = lambda omega, a, omega_0, delta, B, C: fit_func([a, omega_0, delta, B, C], omega) - amplitude_cut_off_freq
